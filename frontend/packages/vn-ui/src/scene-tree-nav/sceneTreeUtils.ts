...
function countDescendants(
    sceneId: string,
    nodesById: Map<string, FlatSceneNode>,
): number {
...
export function buildVisibleSceneGraph(params: {
    ...
    const { tree, collapsedNodeIds, currentSceneId, toggleCollapsed } = params;
        ...
        visibleNodes.push({
            ...
            data: {
                ...
                hiddenDescendantCount: collapsed
                    ? countDescendants(sceneNode.id, nodesById)
                    : 0,
                isCurrent: sceneNode.id === currentSceneId,
                toggleCollapsed,
            },
        });
